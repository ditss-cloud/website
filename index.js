...
// 1. KONEKSI DATABASE AWAL (SYNC TAPI NON-BLOCKING)
...
// 3. LOGGER MIDDLEWARE - HARUS DIPASANG SEBELUM SEMUA ROUTES!
console.log(chalk.blue('📝 Setting up logger middleware...'))
...
const loadApiRoutes = async () => {
  ...
  console.log(chalk.bgGreen.black.bold(` 📊 Total API Routes Loaded: ${totalRoutes} `))
}

// 11. DEBUG ENDPOINT UNTUK TEST LOGGER
app.get("/api/debug/log-test", async (req, res) => {
  console.log(chalk.yellow('🔍 Debug endpoint called'))
  
  // Test koneksi database
  try {
    const { connectDB } = await import('./src/database/db.js')
    await connectDB()
    
    // Test save langsung ke ApiLog
    const { ApiLog } = await import('./src/database/models/ApiLog.js')
    ...
    res.json({
      success: true,
      message: "Manual log test - this should be logged by the logger middleware",
      logId: saved._id,
      timestamp: new Date(),
      loggerActive: true,
      note: "Check MongoDB for ApiLog collection"
    })
...
const startApplication = async () => {
  try {
    console.log(chalk.blue.bold('🚀 Starting Raol-UI Application...'))
    ...
    const server = app.listen(PORT, () => {
      ...
      console.log(chalk.green(` 📍 Docs: http://localhost:${PORT}/docs`))
      console.log(chalk.green(` 📍 Debug: http://localhost:${PORT}/api/debug/log-test`))
    })

    // Start Discord bot jika ada
    try {
      startDiscordBot()
      console.log(chalk.green('🤖 Discord bot started'))
    } catch (error) {
      console.log(chalk.yellow('⚠️ Discord bot not started:', error.message))
    }
    ...
    process.on('SIGTERM', () => {
      ...
      server.close(() => {
        console.log(chalk.green('✅ Process terminated gracefully'))
        process.exit(0)
      })
    })
