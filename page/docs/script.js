...
// Fungsi untuk mapping stats ke endpoint
const mapStatsToEndpoints = (apiStats) => {
  const endpointMap = {};
  
  if (apiStats && apiStats.topEndpointsAllTime) {
    apiStats.topEndpointsAllTime.forEach(stat => {
      const endpoint = stat.endpoint.replace(/^\//, '');
      endpointMap[endpoint] = {
        totalRequests: stat.totalRequests || 0,
        successRate: stat.successRate || '0%',
        avgResponseTime: stat.avgResponseTime || '0ms',
...
// LINE 800: GANTI fungsi init yang lama dengan yang baru
const init = async () => {
  setupEventListeners();
  initTheme();
  initSideNav();
  ...
  try {
    // Load settings dan stats SEKALIGUS
    const [settingsResponse, statsResponse] = await Promise.allSettled([
      fetch("/api/settings").then(res => 
        res.ok ? res.json() : Promise.reject(new Error(`Failed to load settings: ${res.status}`))
      ),
      fetch("https://api.asuma.my.id/admin/stats/alltime").then(res => 
        res.ok ? res.json() : Promise.reject(new Error(`Failed to load stats: ${res.status}`))
      )
    ]);
    ...
    if (settingsResponse.status === "fulfilled") {
      settingsData = settingsResponse.value;
    ...
    if (statsResponse.status === "fulfilled") {
      statsData = statsResponse.value.result;
      // Update statistics section di Hero
      updateStatisticsSection(statsData);
    }

    // LINE 835: Render API dengan data stats
    renderApiCategories(statsData);
  ...
  // LINE 850: Tambah auto-refresh stats
  initAutoRefreshStats();
};

function updateStatisticsSection(statsData) {
  if (!statsData) return;
  
  const summary = statsData.summary;
  
  // Update semua stat cards di hero section
  const elements = {
    'totalEndpoints': summary.uniqueEndpoints,
    'totalRequests': summary.totalRequests,
    'successRate': summary.successRate,
    'uniqueIPs': summary.uniqueIPs,
    'avgResponseTime': summary.avgResponseTime,
    'requestsPerDay': summary.requestsPerDay,
    'totalDays': statsData.period.totalDays,
  ...
  const populatePageContent = () => {
    ...
    const creator = settings.apiSettings?.creator || "Asuma Api'S"

    setPageContent(DOM.pageTitle, settings.name, "Asuma Api'S")
    setPageContent(DOM.wm, `© ${currentYear} Asuma Api'S Corp. All rights reversed.`)
    setPageContent(DOM.appName, settings.name, "Asuma Api'S")
 ...
 // LINE 500: GANTI fungsi renderApiCategories yang lama
const renderApiCategories = (statsData = null) => {
  ...
  // Map stats jika ada
  const endpointStats = statsData ? mapStatsToEndpoints(statsData) : {};
  ...
  settings.categories.forEach((category, categoryIndex) => {
    ...
    // LINE 525: Hitung stats untuk kategori
    let categoryTotalRequests = 0;
    let categoryApis = 0;
    
    sortedItems.forEach(item => {
      const endpointKey = item.path.replace(/^\//, '').split('?')[0];
      if (endpointStats[endpointKey]) {
        categoryTotalRequests += endpointStats[endpointKey].totalRequests || 0;
        categoryApis++;
      }
    });
    
    // Tambah icon kategori
    if (category.icon) {
    ...
    // Badge stats kategori
    if (categoryTotalRequests > 0) {
      const categoryStatsDiv = document.createElement("div");
      categoryStatsDiv.className = "category-stats ms-3";
      categoryStatsDiv.innerHTML = `
        <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25">
          <i class="fas fa-chart-line me-1"></i>
          ${formatNumber(categoryTotalRequests)} uses
        </span>
      `;
      categoryHeader.appendChild(categoryStatsDiv);
    }
    
    categorySection.appendChild(categoryHeader);

    // LINE 550: Lanjutkan seperti biasa...
    const itemsRow = document.createElement("div");
    itemsRow.className = "row";

    sortedItems.forEach((item, itemIndex) => {
      const endpointKey = item.path.replace(/^\//, '').split('?')[0];
      const itemStats = endpointStats[endpointKey];
      ...
      // LINE 570: Title dengan usage badge
      const itemTitle = document.createElement("h5");
      itemTitle.className = "mb-1 d-flex justify-content-between align-items-center";
      ...
      // Usage badge
      if (itemStats && itemStats.totalRequests > 0) {
        const usageBadge = document.createElement("span");
        usageBadge.className = "usage-badge small";
        usageBadge.innerHTML = `<i class="fas fa-chart-bar me-1"></i>${formatNumber(itemStats.totalRequests)}`;
        itemTitle.appendChild(usageBadge);
      }

      const itemDesc = document.createElement("p");
      itemDesc.className = "text-muted mb-2 small";
      itemDesc.textContent = item.desc;
      
      // Stats info jika ada
      if (itemStats) {
        const statsInfo = document.createElement("div");
        statsInfo.className = "api-stats-info mt-2 pt-2 border-top";
          ...
          <div class="row g-2 small">
            <div class="col-6">
              <span class="text-success">
                ...
                ${itemStats.successRate}
              </span>
            </div>
            <div class="col-6 text-end">
              <span class="text-info">
                <i class="fas fa-clock me-1"></i>
                ${itemStats.avgResponseTime}
              </span>
            </div>
          </div>
        `;
        
        cardInfo.appendChild(statsInfo);
      }

      cardInfo.appendChild(itemTitle);
      cardInfo.appendChild(itemDesc);

      // LINE 610: Actions (GET button dan status)
      ...
      getBtn.innerHTML = '<i class="fas fa-code me-1"></i> GET';
      ...
      statusIndicator.innerHTML = `<i class="fas fa-circle me-1"></i><span>${status.charAt(0).toUpperCase() + status.slice(1)}</span>`;
  ...
  // LINE 900: Tambah fungsi auto-refresh
function initAutoRefreshStats() {
  // Refresh stats setiap 30 detik
  setInterval(async () => {
    try {
      const response = await fetch("https://api.asuma.my.id/admin/stats/alltime");
      if (response.ok) {
        const data = await response.json();
        ...
        // Update API cards stats
        updateApiCardsStats(statsData);
        
        console.log("Statistics updated at", getCurrentTime());
      }
    } catch (error) {
...
// LINE 925: Fungsi helper update stats di kartu
function updateApiCardsStats(statsData) {
  const endpointStats = mapStatsToEndpoints(statsData);
  
  document.querySelectorAll('.api-item').forEach(card => {
    const apiName = card.querySelector('h5 span').textContent;
    const apiPath = card.querySelector('.get-api-btn').dataset.apiPath;
    const endpointKey = apiPath ? apiPath.replace(/^\//, '').split('?')[0] : null;
    
    if (endpointKey && endpointStats[endpointKey]) {
      const stats = endpointStats[endpointKey];
      const usageBadge = card.querySelector('.usage-badge');
      
      if (usageBadge) {
        usageBadge.innerHTML = `<i class="fas fa-chart-bar me-1"></i>${formatNumber(stats.totalRequests)}`;
      } else {
        // Buat usage badge jika belum ada
        const titleElement = card.querySelector('h5');
        const newBadge = document.createElement("span");
        newBadge.className = "usage-badge small";
        newBadge.innerHTML = `<i class="fas fa-chart-bar me-1"></i>${formatNumber(stats.totalRequests)}`;
